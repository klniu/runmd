...
class ITPSection(object):
    ...
    - :attr:`sections` holds the sub-parsers (including their own :attr:`data`;
      this :class:`OrderedDict` mirrors the parse tree.
        ...
        while True:
            ...
            if m:
                if current_section == m.group('name'):
                    ...
                    # Note: If we stored sections in a list instead of an OrderedDict then
...
class ITPdata(ITPSection):
    ...
    def _create_recarray(self):
        ...
        dtype = self.dtypes[:nmax] + [("comment", "U128")]
...
class Header(ITPSection):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.parsers = {'atomtypes': Atomtypes, 'moleculetype': Moleculetype}
...
class Atomtypes(ITPdata):
    ...
    """
    name = "atomtypes"
    dtypes = [("name", "U4"), ("bond_type", "U12"), ("mass", "f8"), ("charge", "f8"),
              ("ptype", "U4"), ("sigma", "f8"), ("epsilon", "f8"),
              ]
    fmt = ["%-5s", "%9s", "%9.4f", "%8.4f",
           "%-5s", "%11.5e", "%11.5e"]
...
class Atoms(ITPdata):
    ...
    dtypes = [("atomnr", "i4"), ("atomtype", "U10"), ("resnr", "i4"), ("resname", "U4"),
              ("atomname", "U4"), ("chargegrp", "i4"),
              ("charge", "f8"), ("mass", "f8"),
              ("atomtypeB", "U10"),
              ("chargeB", "f8"), ("massB", "f8"),
              # extra last column ("comment", "U128")
              ]
    ...
    column_comment = ";  nr  type  resnr  resid  atom  cgnr  charge   mass    total_charge"
...
class Bonds(ITPdata):
    ...
    column_comment = ";  ai   aj  funct   c0         c1"
